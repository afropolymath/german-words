...
var author$project$MainView$renderDeck = F3(
	function (deck, activeCardId, cardFrontActive) {
		var displayText = function (card) {
			return cardFrontActive ? card.front : card.back;
		};
			...
			function (cardIndex, card) {
				return A2(
					...
					_List_fromArray(
						[
							elm$html$Html$text(
							elm$core$String$fromInt(cardIndex + 1))
						]));
			});
						...
						[
							...
							_List_fromArray(
								[
									...
									elm$html$Html$Events$onClick(displayPreviousCard)
								]),
							_List_Nil),
							A2(
							elm$html$Html$span,
							_List_fromArray(
								[
									...
									elm$html$Html$Events$onClick(displayNextCard)
								]),
							_List_Nil),
							A2(
							elm$html$Html$h1,
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A2(elm$core$List$indexedMap, deckCardNavItem, deck.cards))
				]));
	});
...
var author$project$MainView$renderDecks = F4(
	function (decks, deckId, cardId, cardFrontActive) {
		if (deckId < 0) {
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								A4(author$project$MainView$renderDecks, model.decks, model.activeDeckId, model.activeCardId, model.cardFrontActive)
							]))
					]))
			]));
};
var author$project$AppTypes$Model = F4(
	function (decks, activeDeckId, activeCardId, cardFrontActive) {
		return {activeCardId: activeCardId, activeDeckId: activeDeckId, cardFrontActive: cardFrontActive, decks: decks};
	});
		...
		switch (msg.$) {
			...
			case 'DisplayCard':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{activeCardId: cardIndex, cardFrontActive: false}),
					elm$core$Platform$Cmd$none);
			case 'FlipCard':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{cardFrontActive: !model.cardFrontActive}),
					elm$core$Platform$Cmd$none);
			default:
...
var author$project$Main$main = elm$browser$Browser$element(
	{init: author$project$State$init, subscriptions: author$project$State$subscriptions, update: author$project$State$update, view: author$project$MainView$view});
