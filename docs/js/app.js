...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.h, request.aZ, true);
		} catch (e) {
...
var author$project$AppTypes$DisplayDeckList = {$: 0};
var author$project$AppTypes$FlipCard = {$: 3};
var author$project$AppTypes$ToggleLearnt = {$: 4};
...
var author$project$MainView$alwaysStop = function (x) {
...
var author$project$MainView$stopPropagationOnClick = function (msg) {
	return A2(
		elm$html$Html$Events$stopPropagationOn,
		'click',
		A2(
			elm$json$Json$Decode$map,
			author$project$MainView$alwaysStop,
...
var author$project$MainView$renderDeck = F2(
	function (model, deck) {
		var displayText = function (card) {
			return model.ay ? card.aw : card.aF;
		};
		var displayPreviousCard = (!model.ar) ? author$project$AppTypes$DisplayCard(model.ar) : author$project$AppTypes$DisplayCard(model.ar - 1);
		var displayNextCard = _Utils_eq(
			model.ar,
			elm$core$List$length(deck.az) - 1) ? author$project$AppTypes$DisplayCard(model.ar) : author$project$AppTypes$DisplayCard(model.ar + 1);
						...
						[
							...
							_List_fromArray(
								[
									_Utils_Tuple2(
									'active',
									_Utils_eq(cardIndex, model.ar)),
									_Utils_Tuple2('learnt', card.aK)
								])),
		...
		var activeCard = function () {
			var _n0 = A2(elm_community$list_extra$List$Extra$getAt, model.ar, deck.az);
			if (!_n0.$) {
				var card = _n0.a;
				return A2(
					elm$html$Html$div,
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									_Utils_Tuple2('is-learnt', card.aK)
								]))
						]),
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									elm$html$Html$Attributes$class('control-left'),
									author$project$MainView$stopPropagationOnClick(displayPreviousCard)
								]),
							_List_Nil),
							A2(
							elm$html$Html$span,
							_List_fromArray(
								[
									elm$html$Html$Attributes$class('control-right'),
									author$project$MainView$stopPropagationOnClick(displayNextCard)
								]),
							_List_Nil),
							A2(
							elm$html$Html$h1,
				...
				return A2(
					...
					_List_fromArray(
						[
							elm$html$Html$text('Could not find that card')
						]));
			}
		}();
		return A2(
			...
			_List_fromArray(
				[
					elm$html$Html$Attributes$class('deck')
				]),
			_List_fromArray(
				[
					A2(
					...
					_List_fromArray(
						[
							elm$html$Html$Attributes$class('deck-card-navigation')
						]),
					A2(elm$core$List$indexedMap, deckCardNavItem, deck.az))
				]));
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				elm$html$Html$Events$onClick(
				author$project$AppTypes$DisplayDeck(deck.aT))
			]),
		_List_fromArray(
			[
				A2(
	...
	function (userDecks, applicationDecks) {
		var classifyBeforeRender = function (deck) {
			return A2(
				...
				deck,
				A2(author$project$Utils$findBySlug, deck.aT, userDecks));
		};
		return A2(
			elm$html$Html$div,
			_List_fromArray(
	...
	if (_n0.$ === 3) {
		...
		if (!_n1.$) {
			...
			var _n2 = A2(
				...
				A2(author$project$Utils$findBySlug, activeDeckSlug, model.a_));
			if (!_n2.$) {
				var activeDeck = _n2.a;
				return A2(author$project$MainView$renderDeck, model, activeDeck);
			} else {
				return A2(author$project$MainView$renderDeckList, model.a_, applicationDecks);
			}
		} else {
			return A2(author$project$MainView$renderDeckList, model.a_, applicationDecks);
		}
	} else {
		return A2(author$project$MainView$renderDeckList, model.a_, _List_Nil);
	}
};
...
var author$project$AppTypes$Model = F6(
	function (applicationTitle, applicationDecks, userDecks, activeDeckSlug, activeCardId, cardIsFlipped) {
		return {ar: activeCardId, as: activeDeckSlug, at: applicationDecks, au: applicationTitle, ay: cardIsFlipped, a_: userDecks};
	});
...
var elm$http$Http$request = function (r) {
	return elm$http$Http$command(
		elm$http$Http$Request(
			{v: false, ax: r.ax, aC: r.aC, e: r.e, h: r.h, i: r.i, j: r.j, aZ: r.aZ}));
};
var elm$http$Http$get = function (r) {
	return elm$http$Http$request(
		{ax: elm$http$Http$emptyBody, aC: r.aC, e: _List_Nil, h: 'GET', i: elm$core$Maybe$Nothing, j: elm$core$Maybe$Nothing, aZ: r.aZ});
};
...
var author$project$Ports$cacheDecks = _Platform_outgoingPort(
	'cacheDecks',
			...
			return elm$json$Json$Encode$object(
				_List_fromArray(
					[
						...
						_Utils_Tuple2(
						'slug',
						elm$json$Json$Encode$string($.aT)),
						_Utils_Tuple2(
						'tags',
						elm$json$Json$Encode$list(elm$json$Json$Encode$string)($.aW))
					]));
		}));
		...
		var updatedCards = A3(
			...
			F2(
				function (appDeckCard, userDeckCard) {
					return _Utils_update(
						userDeckCard,
						{aw: appDeckCard.aw, aF: appDeckCard.aF});
				}),
			applicationDeck.az,
			deck.az);
		var deckCardLength = elm$core$List$length(deck.az);
		var cardsAppendage = A2(elm$core$List$drop, deckCardLength, applicationDeck.az);
		return _Utils_update(
			deck,
			{
				az: _Utils_ap(updatedCards, cardsAppendage),
				aL: applicationDeck.aL,
				aW: applicationDeck.aW
			});
	});
...
var author$project$Utils$replaceBySlug = F3(
	function (slug, replacementDeckFn, deck) {
		return _Utils_eq(slug, deck.aT) ? replacementDeckFn(deck) : deck;
	});
			...
			if (!_n1.$) {
				...
				if (!_n2.$) {
					...
					return onlySync ? A2(
						...
						model.a_) : ((!A2(author$project$Utils$isDeckInList, activeDeckSlug, model.a_)) ? _Utils_ap(
						model.a_,
						_List_fromArray(
							[applicationDeck])) : model.a_);
				} else {
					return model.a_;
				}
			} else {
...
var author$project$State$reducerDisplayDeck = F2(
	function (model, deckSlug) {
		return _Utils_update(
			...
			{
				ar: 0,
				as: elm$core$Maybe$Just(deckSlug),
				a_: A2(
...
var author$project$State$reducerDisplayDeckList = function (model) {
	return _Utils_update(
		model,
		{ar: -1, as: elm$core$Maybe$Nothing});
};
var author$project$State$reducerFlipCard = function (model) {
	return _Utils_update(
		...
		{
			ay: !model.ay,
			a_: (model.ar > 0) ? A2(author$project$State$updatedUserDecks, model, false) : model.a_
		});
};
	...
	function (model, cardUpdateFunc) {
		var updateCard = F2(
			function (cardIterIndex, card) {
				return _Utils_eq(cardIterIndex, model.ar) ? cardUpdateFunc(card) : card;
			});
		var updateDeck = function (deck) {
			var _n0 = model.as;
			if (!_n0.$) {
				var activeDeckSlug = _n0.a;
				return _Utils_eq(deck.aT, activeDeckSlug) ? _Utils_update(
					deck,
					{
						az: A2(elm$core$List$indexedMap, updateCard, deck.az)
					}) : deck;
			} else {
				return deck;
			}
...
var author$project$State$updateWithStorage = F2(
	function (msg, model) {
		...
		return _Utils_Tuple2(
			...
			elm$core$Platform$Cmd$batch(
				_List_fromArray(
					[
						commands,
						author$project$Ports$cacheDecks(newModel.a_)
					])));
	});
...
var author$project$Main$main = elm$browser$Browser$element(
	{aJ: author$project$State$init, aV: author$project$State$subscriptions, aY: author$project$State$updateWithStorage, a$: author$project$MainView$view});
											...
											return A2(
												...
												function (cards) {
													return elm$json$Json$Decode$succeed(
														{az: cards, aL: name, aT: slug, aW: tags});
												},
												A2(
